...
public interface CustomerRatingRepository extends JpaRepository<CustomerRating, Long> {
    ...
    @Query(value = "select m.id, m.title, avg(c.rating) as rating, m.average_rating, m.image_url from movie m join customer_rating c on c.movie_id = m.id and  rating_time between ?1 AND ?2 group by m.id order by rating desc limit 10", nativeQuery = true)
